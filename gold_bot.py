...
def run_backtest_live(): while True: cerebro = bt.Cerebro() logging.info("Fetching market data...") df = yf.download(SYMBOL, period=data_period, interval=data_interval) df.columns = [c.lower() for c in df.columns] data = bt.feeds.PandasData(dataname=df)
